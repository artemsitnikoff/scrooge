...
async def _send_invoice(callback: CallbackQuery, bot: Bot, object_pk: int, plan: str) -> None:
    ...
    if plan == "year":
        title = "SCROOGE — подписка на объект"
        description = (
            f"{obj['name']} · 12 месяцев · безлимитные передачи в ФГИС УТКО\n"
            "Экономия 5 800 ₽ — 2 месяца бесплатно"
        )
        amount = _YEAR_PRICE
        label = "Подписка 12 месяцев"
    else:
        title = "SCROOGE — подписка на объект"
        description = f"{obj['name']} · 1 месяц · безлимитные передачи в ФГИС УТКО"
        amount = _MONTH_PRICE
